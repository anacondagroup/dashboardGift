...
describe.skip('ChooseCampaignOwnerSection', () => {
  ...
  it('Should populate passed values into form fields', () => {
    const data = {
      owner: {
        id: 100,
        name: 'Batch owner',
      ...
      codesBatchName: 'Batch name',
    };
    const { getByText, getBatchOwnerField } = setup({ data });

    expect(getBatchOwnerField()).toHaveValue(data.codesBatchName);
    expect(getByText(data.owner.name)).toBeInTheDocument();
  });
});
