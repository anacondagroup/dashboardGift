...
import {
  ...
  swagSelectUpdateCardOrderDataRequest,
...
const CardOrderOptionsSection = ({ title, order, status, data, campaignId, isLoading }) => {
  ...
  const [codesBatchName, setBatchName] = useState(data.codesBatchName || '');
  ...
  const handleBatchNameField = useCallback(
    value => {
      const checkedValue = value && value.trimLeft();
      validate(checkedValue, 'codesBatchName');
      setBatchName(checkedValue);
    },
    [validate, setBatchName],
  );
  ...
  const handleNextStep = useCallback(() => {
    ...
    dispatch(swagSelectSetStepData({ step: SS_CARD_ORDER_OPTIONS_STEP, data: mergeRight(data, componentData) }));
    dispatch(swagSelectUpdateCardOrderDataRequest(campaignId));
  }, [codesBatchName, contactName, deliveryAddress, codesAmount, codesExpirationDate, data, dispatch, campaignId]);
  ...
  const isNextDisabled = useMemo(
    ...
    [codesAmount, codesBatchName, codesExpirationDate, contactName, deliveryAddress, errors],
  );
  ...
  if (status === SWAG_SELECT_FLOW_STATES.COMPLETED) {
    return (
      <CompletedSection
        ...
        multiline
      >
        {`${codesBatchName} batch has ${codesAmount} cards and it will ship to ${addressToString(deliveryAddress)}`}
      </CompletedSection>
    );
  }
  ...
  return (
    <Box width={648}>
      ...
      <Box pt={1} px={3}>
        ...
        <TextField
          fullWidth
          error={!!(errors && errors.codesBatchName)}
          helperText={errors && errors.codesBatchName}
          label="Batch name"
          variant="outlined"
          margin="normal"
          value={codesBatchName}
          onChange={e => handleBatchNameField(e.target.value)}
        />
