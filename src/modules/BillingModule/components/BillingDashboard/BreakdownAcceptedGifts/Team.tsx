...
const Team = ({ team, group, isExpandedByDefault }: ITeamProps) => {
  ...
  const { teamId, teamName, totalMoney, totalInvites, resources } = team;
  ...
  const handleClick = useCallback(() => {
    if (canBeExpanded) {
      ...
      if (!isExpanded) {
        trackEvent('Billing Insights - Gifts accepted - Team - Expanded', { groupId, teamId });
      }
    }
  ...
  return (
    <>
      <TableRow onClick={handleClick} className={classes.tableRow}>
        <TableCell padding="none" className={classNames(classes.tableCell, classes.nameCell)}>
          <TableCellTooltip title={teamName} lengthToShow={32} />
        </TableCell>
        <TableCell
          align="right"
          className={classes.tableCell}
          data-testid={formatTestId(`BillingInsight.AcceptedSum.${groupName}.${teamName}`)}
        >
          <NumberFormat>{totalMoney}</NumberFormat>
        </TableCell>
        <TableCell
          align="right"
          className={classes.tableCell}
          data-testid={formatTestId(`BillingInsight.AcceptedCount.${groupName}.${teamName}`)}
        >
        ...
        <TableCell align="right" padding="checkbox" className={classes.tableCell}>
          {canBeExpanded ? (
            <ExpandIcon
              ...
              data-testid={formatTestId(`BillingInsight.Accepted.Expand.${groupName}.${teamName}`)}
            />
          ) : null}
        </TableCell>
      </TableRow>
