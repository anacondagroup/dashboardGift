...
import { CommonData, Currencies, CurrencyConverter, SFormLabel } from '@alycecom/modules';
...
export interface IDigitalProductTypeBudgetProps {
  ...
  name: MarketplaceDataFields.GiftCardMaxBudget | MarketplaceDataFields.DonationMaxBudget;
  label: string;
  fieldLabel: string;
  converter?: boolean;
...
const DigitalProductTypeBudget = ({
  ...
  const productType = name === MarketplaceDataFields.GiftCardMaxBudget ? ProductTypes.eGift : ProductTypes.donation;
  const checkboxName =
  ...
  const checkboxControllerName = `${GiftingStepFields.ExchangeMarketplaceSettings}.${checkboxName}` as const;
  const textFieldName = `${GiftingStepFields.ExchangeMarketplaceSettings}.${name}` as const;
  ...
  const restrictedGiftTypeIds = useRestrictedIds();
  ...
  const isRestricted = useMemo(() => restrictedGiftTypeIds.includes(productType), [productType, restrictedGiftTypeIds]);
  ...
  const {
    ...
    control,
    name: `${GiftingStepFields.ExchangeMarketplaceSettings}.${MarketplaceDataFields.RestrictedGiftTypeIds}` as const,
  });
        ...
        <Grid item display="flex">
          <FormControl error={!!restrictedTypeError?.message}>
            <FormControlLabel
              ...
              label={<SFormLabel>{label}</SFormLabel>}
            />
          </FormControl>
